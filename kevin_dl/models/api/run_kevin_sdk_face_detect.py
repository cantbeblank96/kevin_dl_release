...
def run_kevin_sdk_face_detect(image_ls=None, input_dir=None, output_contents=("landmarks", "face_bbox"), **kwargs):
    ...
    detect_model_path = kwargs.get(
        "detect_model_path",
        os.path.join(setting_s["models_dir"], "xxx.model")
    )
    align_model_path = kwargs.get(
        "align_model_path",
        [os.path.join(setting_s["models_dir"], "xxx.model"),
         os.path.join(setting_s["models_dir"], "xxx.model")]
    )
    align_model_type = kwargs.get("align_model_type", ["360align", "align"])
